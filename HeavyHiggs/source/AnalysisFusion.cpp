...
std::string AnalysisFusion::ProcessName(const AnalysisFusion::ProcessType Process) const
{
    switch (Process) {
    case Hbb:
        return "H0bb-ljbbbb";
...
std::string AnalysisFusion::ColliderName(const AnalysisFusion::ColliderType Collider) const
{
    switch (Collider) {
    case LHC :
        return "14TeV";
...
float AnalysisFusion::BackgroundCrosssection(const AnalysisFusion::ProcessType Process) const
{
    ...
    default:
        Error("Background Crosssection",  "unhandled pre cut", PreCut());
        return 1;
    }
}
std::string AnalysisFusion::BackgroundTree(const AnalysisFusion::ProcessType Process) const
{
    return ProcessName(Process) + "-" + ColliderName(collider_type()) + "-" + std::to_string(PreCut()) + "GeV_0-run_01";
}
File AnalysisFusion::BackgroundFile(const AnalysisFusion::ProcessType Background, const int FileSum) const
{
...
File AnalysisFusion::BackgroundFile(const AnalysisFusion::ProcessType Background) const
{
    return BackgroundFile(Background, BackgroundFileNumber());
}
float AnalysisFusion::LeptonPt()
{
    switch (collider_type()) {
    case LHC :
        return 50;
...
float AnalysisFusion::MissingEt()
{
    switch (collider_type()) {
    case LHC :
        return 30;
...
float AnalysisFusion::SignalCrosssection() const
{
    switch (collider_type()) {
    case LHC:
        switch (Mass()) {
...
int AnalysisFusion::BackgroundFileNumber() const
{
    switch (collider_type()) {
    case LHC :
        switch (PreCut()) {
...
int AnalysisFusion::EventNumberMax() const
{
//                 return 1000000;
//         return 100000;
    return 10000;
...
AnalysisFusion::ColliderType AnalysisFusion::collider_type() const
{
//         return LHC;
    //       return FHC;
    return LE;
}
int AnalysisFusion::PreCut() const
{
    switch (collider_type()) {
    case LHC :
        switch (Mass()) {
...
std::string AnalysisFusion::ProjectName() const
{
