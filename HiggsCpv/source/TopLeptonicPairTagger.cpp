...
int TopLeptonicPairTagger::Train(const Event& event, analysis::PreCuts&, const analysis::Tag tag) const
{
    ...
    if (top_particles.size() != 2 && tag == Tag::signal)
        Error("Number of Tops?", particles.size());
    std::vector<Triplet> final_triplets;
    switch (tag) {
    case Tag::signal :
        for (const auto& doublet : triplet) for (const auto top : top_particles) if (doublet.Coincides(top))
                    final_triplets.emplace_back(doublet);
        break;
    case Tag::background :
