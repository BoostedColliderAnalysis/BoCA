...
public:
    ...
    float DistanceToSegment(Vector2<Value> const& point) const {
    ...
    float DistanceToLine(Vector2<Value> const& point) const {
        if (Mod2() == 0) return Distance_1(point); // v == w case
    ...
    float Distance_1(Vector2<Value> const& point) const {
        return Vector_1(point).Mod();
    }
    float Distance_2(Vector2<Value> const& point) const {
        return Vector_2(point).Mod();
    }
    float Paramter(Vector2<Value> const& point) const {
    ...
    float Area(Vector2<Value> const& point) const {
