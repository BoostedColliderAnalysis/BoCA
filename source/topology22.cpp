...
void event22::cubic(double *a, double *x, int &l)
{
    double u[3];
    ...
    if (a[3] != 0.) {
        ...
        double phi;
        if (dis < 0.) {
            phi = acos(std::min(1., std::max(-1., q / sqrt(-p))));
            p   = 2.*sqrt(cbrt(-p));
...
//linear equation
    ...
    for (int i = 0; i < l; i ++)
        x[i] = x[i] - (a[0] + x[i] * (a[1] + x[i] * (a[2] + x[i] * a[3]))) /
               (a[1] + x[i] * (2.*a[2] + x[i] * 3.*a[3]));
}

void event22::quartic(double *dd, double *sol, double *soli, int &nsol)
{
    double aa[4], z[3];
    int i;
    nsol = 0;
...
void event22::solve22(event22 &evt, double mn, double mx, double my, int &nsols, double p1[][4], double p2[][4])
{
    ...
    cxe1 =
        (-((-4 * e5 * p3z * p4z * p6y + 4 * e3 * p4z * p5z * p6y +
    ...
    cxe2 =
        (-((-4 * e6 * p3z * p4z * p5y + 4 * e6 * p3y * p4z * p5z +
    ...
    cze1 =
        ((e5 * (p3y * p4z * p6x - p3x * p4z * p6y - p3y * p4x * p6z +
                p3x * p4y * p6z) +
    ...
    czte1 =
        ((e5 * (-(p3z * p4y * p6x) + p3y * p4z * p6x + p3z * p4x * p6y -
    ...
    czt =
        (del3b * (p3z * p4y * p5x - p3y * p4z * p5x - p3z * p4x * p5y +
                  p3x * p4z * p5y + p3y * p4x * p5z - p3x * p4y * p5z) +
    ...
    cf =
        a11 * a11 * a22 * b1 * b1 + a11 * a12 * a12 * b * b11 -
    ...
    df =
        -(a11 * a11 * a12 * b * b1) + a11 * a11 * a2 * b1 * b1 +
