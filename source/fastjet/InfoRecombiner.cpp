...
void InfoRecombiner::recombine(fastjet::PseudoJet const &jet_1, fastjet::PseudoJet const &jet_2, fastjet::PseudoJet &jet) const
{
    ...
    if (jet_1.has_user_info<JetInfo>() && jet_2.has_user_info<JetInfo>()) jet.set_user_info(new JetInfo(jet_1.user_info<JetInfo>() + jet_2.user_info<JetInfo>()));
    else {
        if (jet_1.has_user_info<JetInfo>()) jet.set_user_info(new JetInfo(jet_1.has_user_info<JetInfo>()));
        if (jet_2.has_user_info<JetInfo>()) jet.set_user_info(new JetInfo(jet_2.has_user_info<JetInfo>()));
    }
