...
fastjet::PseudoJet analysis::HHiggsTagger::GetHiggsJet(const Jets &EFlowJets, const Jets &BottomJets, const Jets &CharmJets)
{
    Print(kInformation, "GetHiggsJet");
    ...
    for (auto & FatJet : GetFatJets(EFlowJets)) {
        ...
        if (MassDropJet == 0) {
            ...
            Print(kDebug, "No substructure found");
            continue;
...
// {
//
//     Print(kInformation, "GetFatJetVector");
//
...
//     Print(kInformation,"Number of Fat Jets",FatJetVector.size());
//
...
fastjet::PseudoJet analysis::HHiggsTagger::GetFilteredJet(const fastjet::PseudoJet &MassDropJet, const fastjet::JetAlgorithm &FilterJetAlgorithm, const int NumberHardestPieces)
{

    Print(kInformation, "GetFilteredJet");

    Jets MassDropPieces = sorted_by_E(MassDropJet.pieces());
    if (MassDropPieces.size() != 2) Print(kError, "Number of Subjets", MassDropPieces.size());
...
//     const int NumberHardestPieces = 3;
    ...
    Print(kDebug, "SubJets filtered");

    return FilteredJet;

}

void analysis::HHiggsTagger::GetSubJetSource(const Jets &Particles, const int UserIndex)
{

    Print(kInformation, "GetSubJetSource");
...
//     const int PieceSum = 2;
    for (unsigned PieceNumber = 0; PieceNumber < FilteredJetPieces.size(); ++PieceNumber) {
        ...
        if (PieceRap < SubJetRapMax && PiecePt > SubJetPtMin) {
            ...
            Print(kDebug, "SubJet kinematics are fine");
            ...
            if ((PieceUserIndex == BottomUserIndex && RandomPerMil < BottomPerMil) ||
                ...
                Print(kDebug, "Subjet tagged");

            } else {

                Print(kDebug, "SubJet not tagged");

            }
        } else {

            Print(kDebug, "SubJet has bad kinematics");
...
float analysis::HHiggsTagger::GetDipolarity(const fastjet::PseudoJet &FatJet)
{
    Print(kInformation, "GetDipolarity");
...
//     const int NumberHardestPieces = 2;
    ...
    if (SubJets.size() != 2) Print(kError, "Number of SubJets", SubJets.size());
    ...
    Print(kDebug, "Number of constituents", constituents.size());
...
void analysis::HHiggsTagger::SetRapPhi(fastjet::PseudoJet &SubJet1, fastjet::PseudoJet &SubJet2)
{
    Print(kInformation, "SetRapPhi");
...
float analysis::HHiggsTagger::GetSubDeltaR()
{
    ...
    Print(kInformation, "GetSubDeltaR");
