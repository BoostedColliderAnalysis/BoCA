...
analysis::HeavyHiggsLeptonicTagger::HeavyHiggsLeptonicTagger()
{
...
void analysis::HeavyHiggsLeptonicTagger::DefineVariables()
{
...
int analysis::HeavyHiggsLeptonicTagger::Train(Event &event, const Object::Tag tag)
{
...
std::vector<analysis::Sextet>  analysis::HeavyHiggsLeptonicTagger::Multiplets(Event &event, const TMVA::Reader &reader)
{
    ...
    for (const auto & doublet1 : doublets) {
        for (const auto & doublet2 : doublets) {
            ...
            Quartet22 quartet(doublet1, doublet2);
            std::vector<analysis::Sextet> Presextets;
            Presextets = Sextets(quartet, missing_et);
...
void analysis::HeavyHiggsLeptonicTagger::SetMomentum(double momentum[4], const fastjet::PseudoJet &jet)
{
    momentum[0] = jet.E();
    momentum[1] = jet.px();
    momentum[2] = jet.py();
...
std::vector<analysis::Sextet> analysis::HeavyHiggsLeptonicTagger::Sextets(const Quartet22 &quartet, const fastjet::PseudoJet &missing_et)
{
...
std::vector<analysis::Sextet> analysis::HeavyHiggsLeptonicTagger::sextet(const Quartet22 &quartet, const fastjet::PseudoJet &missing_et, const Jets &neutrinos, const Tag tag)
{
