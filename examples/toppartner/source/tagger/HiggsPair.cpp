...
int HiggsPair::Train(boca::Event const& event, PreCuts const&, Tag tag)
{
    INFO0;
...
std::vector<Quartet22> HiggsPair::Multiplets(boca::Event const& event, boca::PreCuts const&, TMVA::Reader const& reader)
{
    INFO0;
...
std::vector<std::pair<Particle, Particle>> HiggsPair::Particles(boca::Event const& event, Tag tag) const
{
...
std::vector<Quartet22> HiggsPair::Quartets(boca::Event const& event, std::function<Quartet22(Quartet22&)> const& function)
{
    INFO0;
...
std::string HiggsPair::Name() const
{
    return "HiggsPair";
}

latex::String HiggsPair::LatexName() const
{
    return {"hh", true};
}
