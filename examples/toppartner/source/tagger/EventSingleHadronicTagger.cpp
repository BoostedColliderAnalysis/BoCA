...
int EventSingleHadronicTagger::Train(boca::Event const& event, PreCuts const&, Tag tag)
{
    INFO0;
...
std::vector<EventMultiplet<Decuplet532>> EventSingleHadronicTagger::Events(boca::Event const& event, std::function<EventMultiplet<Decuplet532>(boca::EventMultiplet<Decuplet532> &)> const& function)
{
    INFO0;
...
latex::String EventSingleHadronicTagger::LatexName() const
{
    return "T_{h}t_{l}h";
}
