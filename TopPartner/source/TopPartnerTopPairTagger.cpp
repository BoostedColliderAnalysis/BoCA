...
int TopPartnerTopPairTagger::Train(const Event& event, PreCuts& pre_cuts, const Tag tag) const
{
    Info();
...
std::vector<Octet53> TopPartnerTopPairTagger::Multiplets(const Event& event, analysis::PreCuts& pre_cuts, const TMVA::Reader& reader) const
{
    std::vector<Quintet> quintets = top_partner_reader_.Multiplets(event);
    std::vector<Triplet> triplets = top_reader_.Multiplets(event);
    std::vector<Octet53> octets;
