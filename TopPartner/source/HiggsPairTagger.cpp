...
int HiggsPairTagger::Train(const Event& event, const PreCuts& pre_cuts, const Tag tag) const
{
    Info("Higgs Tags");
...
std::vector<Quartet22> HiggsPairTagger::Multiplets(const Event& event, const analysis::PreCuts& pre_cuts, const TMVA::Reader& reader) const
{
    std::vector<Doublet> doublets = higgs_reader_.Multiplets(event);
    std::vector<Quartet22> quartets;
    for (auto doublet_1 = doublets.begin(); doublet_1 != doublets.end(); ++doublet_1) {
